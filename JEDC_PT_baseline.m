...
tstack_PT = [];           %vector containing taxes that meet the govt. spending target
stackmcy_PT = [];
stackmco_PT = [];
stackvcy_PT = [];
stackvco_PT = [];
...
stackmk_PT = [];
stackmy_PT = [];
WG_PT = [];               %welfare gain (or loss) in % aggregate consumption relative to zero indexation 
...
%Indexation share loop (outer loop)
   ...
   %Tax loop (inner loop)         
    ...
    if i==1    
        t = tinit + 0.00000012*j;
    elseif i>1    
        t = tend - 0.00000105 + 0.00000012*j;    
        %sets tax at start of loop close to value that met govt spending target for previous indexation share (far more efficient) 
    end
    
      
%% ========================================================================
% 2. Preparing the olg_zin_index_PT_total.mod file
%==========================================================================

delete olg_zin_index_PT_total.mod                                      
%Otherwise the new stuff keeps on getting appended to old stuff.
!type olg_zin_index_PT_block_begin.mod >> olg_zin_index_PT_total.mod;
%this puts the *_begin.mod file into *_total.mod
temp = sprintf('echo t = %d; >> olg_zin_index_PT_total.mod',t);
system(temp);
temp = sprintf('echo sharei = %d; >> olg_zin_index_PT_total.mod',sharei);
system(temp);
%this creates a line that sets the value of sharei and adds it to *_total.mod
!type olg_zin_index_PT_block_end.mod >> olg_zin_index_PT_total.mod;
%this puts the *_end.mod file into *_total.mod
temp = sprintf('echo vcov = [%d %d %d; %d %d %d; %d %d %d]; >> olg_zin_index_PT_total.mod',vcov);
system(temp);
...
temp = sprintf('%s\\dynare++ --order 2 --sim 100 --per 1100 --burn 100 --no-irfs olg_zin_index_PT_total.mod',...
    mydynareppdirectory);
system(temp);
load 'olg_zin_index_PT_total.mat'

stack(i,j) = dyn_mean(14);
...
delete olg_zin_index_PT_total.jnl
delete olg_zin_index_PT_total_f.m
delete olg_zin_index_PT_total_ff.m
delete olg_zin_index_PT_total.dump

        end

        %Indexation and real variables
        tend = t;   %last tax in loop (satisfies govt spending target)
        tstack_PT = [tstack_PT; tend];   %vector of taxes that satisfy govt spending target (one for each indexation share)
        stackmcy_PT = [stackmcy_PT; dyn_mean(3)];   %Corresponding mean consumption by the young
        stackmco_PT = [stackmco_PT; dyn_mean(29)];  %Corresponding mean consumption by the old
        stackvcy_PT = [stackvcy_PT; dyn_vcov(3,3)]; %Variance of consumption by the young 
        stackvco_PT = [stackvco_PT; dyn_vcov(29,29)];   %Variance of consumption by the old
        irpdiffstack_PT = [irpdiffstack_PT; dyn_mean(16)];  %Average difference between inflation risk premium on nominal and indexed bonds
        stackri_PT = [stackri_PT; dyn_mean(15)];    %Mean return on indexed bonds
        stackrn_PT = [stackrn_PT; dyn_mean(7)];     %Mean return on nominal bonds
        stackvri_PT = [stackvri_PT; dyn_vcov(15,15)];   %Variance of return on indexed bonds
        stackvrn_PT = [stackvrn_PT; dyn_vcov(7,7)];     %Variance of return on nominal bonds
        stackmk_PT = [stackmk_PT; dyn_mean(18)];    %Mean capital
        stackmy_PT = [stackmy_PT; dyn_mean(8)];    %Mean output
...
WG_PT(i) = 100*( (utility(i)/utility(1))^(1/(1-RA)) - 1 );
...
figure (2)
    plot(stacki, gmean_PT);
    title('Mean g','fontsize',10)
       
figure(3)
    hold on,
   
    subplot(3,4,1); plot(stacki, stackmcy_PT);
    title('Mean C by young','fontsize',10)
    
    subplot(3,4,2); plot(stacki, stackmco_PT);
    title('Mean C by old','fontsize',10)
    
    subplot(3,4,3); plot(stacki, stackvcy_PT);
    title('Var C by young','fontsize',10)
    
    subplot(3,4,4); plot(stacki, stackvco_PT);
    title('Var C by old','fontsize',10)
    
    subplot(3,4,5); plot(stacki, stackri_PT);
    title('Mean return indexed','fontsize',10)
    
    subplot(3,4,6); plot(stacki, stackrn_PT);
    title('Mean return nominal','fontsize',10)
    
    subplot(3,4,7); plot(stacki, stackvri_PT);
    title('Var return indexed','fontsize',10)
    
    subplot(3,4,8); plot(stacki, stackvrn_PT);
    title('Var return nominal','fontsize',10)
    
    subplot(3,4,9); plot(stacki, irpdiffstack_PT);
    title('IRP diff','fontsize',10)
    
    subplot(3,4,10); plot(stacki, tstack_PT);
    title('Taxes','fontsize',10)
    
    subplot(3,4,11); plot(stacki, stackmk_PT);
    title('Capital','fontsize',10)
    
    subplot(3,4,12); plot(stacki, stackmy_PT);
